...
from type_valuev3 import Closure, Object, Type, Value, create_value, get_printable
...
class Interpreter(InterpreterBase):
    ...
    def __get_func_by_name(self, name, num_params):
        if name not in self.func_name_to_ast:
            ...
            if '.' in name:
                closure_val_obj = self.__eval_obj_field(name)
    ...
    def __call_method(self, call_ast):
        ...
        target_closure = self.__get_func_by_name(obj_name + '.' + method_name, len(actual_args))

        if target_closure == None:
            super().error(ErrorType.NAME_ERROR, f"Function {method_name} not found")
        if target_closure.type != Type.CLOSURE:
            super().error(ErrorType.TYPE_ERROR, f"Function {method_name} is changed to non-function type.")
        target_ast = target_closure.func_ast

        new_env = {}
        new_env["this"] = self.__name_to_obj(obj_name)
        self.__prepare_env_with_closed_variables(target_closure, new_env)
        self.__prepare_params(target_ast,call_ast, new_env)
        self.env.push(new_env)
        _, return_val = self.__run_statements(target_ast.get("statements"))
    ...
    def __assign(self, assign_ast):
        ...
        if '.' in var_name:
            #print(f"accessing field {var_name}")
            self.__set_obj_field(var_name, src_value_obj)
            return
    ...
    def __set_obj_field(self, name, src_value_obj):
        name, field = name.split('.')
        target_value_obj = self.__name_to_obj(name).value()
        #print(target_value_obj.dict)

        #if field is proto
        if field == "proto" and src_value_obj.t != Type.NIL and src_value_obj.t != Type.OBJECT:
            super().error(
                ErrorType.TYPE_ERROR,
                "passing non-nil/non-object into proto",
            )

        if field not in target_value_obj.dict:
            target_value_obj.dict[field] = src_value_obj
            #print(target_value_obj.dict[field].value())
        else:
            #print(target_value_obj.dict[field])
            if target_value_obj.dict[field].t == Type.CLOSURE and src_value_obj.t != Type.CLOSURE:
               target_value_obj.dict[field].v.type = src_value_obj.t
            target_value_obj.dict[field] = src_value_obj 


    def __eval_expr(self, expr_ast):
        #print(expr_ast.elem_type)
        ...
        if expr_ast.elem_type == Interpreter.LAMBDA_DEF:
            return Value(Type.CLOSURE, Closure(expr_ast, self.__capture_env(self.env)))
        if expr_ast.elem_type == Interpreter.OBJ_DEF:
            return Value(Type.OBJECT, Object())

    def __capture_env(self, env):
        #print(f"env that got copied: {env.environment}")
        env = copy.deepcopy(env)
        
        for x in env:
            if x[1].t == Type.OBJECT or x[1].t == Type.CLOSURE:
                for layer in reversed(env.environment):
                    if x[0] in layer:
                        layer[x[0]] = self.env.get(x[0])
                        break
        #print(f"env after copy: {env.environment}")
        return env
    ...
    def __eval_name(self, name_ast):
        ...
        if '.' in var_name:
            return self.__eval_obj_field(var_name)
    ...
    def __eval_obj_field(self, name, obj=None):
        obj_name, field = name.split('.')
        value_obj = self.__name_to_obj(obj_name).value()
        if obj:
            value_obj = obj.value()


        if field not in value_obj.dict:
            if value_obj.dict["proto"].t == Type.NIL:
                super().error(
                    ErrorType.NAME_ERROR,
                    "field does not exist in object/prototype",
                )
            return self.__eval_obj_field(name, value_obj.dict["proto"])
        
        if field == "proto" and value_obj.dict[field].t == Type.NIL:
            super().error(
                ErrorType.NAME_ERROR,
                "object's prototype is not defined",
            ) 
        return value_obj.dict[field]
    ...
    def __name_to_obj(self, name):
        obj = self.env.get(name)
        if obj is None:
            super().error(
                ErrorType.NAME_ERROR,
                "obj name not in environment",
            )
        if obj.type() != Type.OBJECT:
            super().error(
                ErrorType.TYPE_ERROR,
                "calling dot member on non-object",
            )
        return obj
